...
def bin_data(hubway_row):
    if hubway_row['num_bikes'] < 16:
        return hubway_row['num_bikes']
    else:
        return 16
...
for station_number in get_station_ids():
    ...
    with open('../findmyride_models/rfc_all_%d.pkl' %(station_number), 'wb') as f:
        pickle.dump(best_rf_clf, f)
    with open('../findmyride_models/rfc_all_cm_%d.pkl' %(station_number), 'wb') as f:
